    ...
    secrix.services =
      lib.mapAttrs' (
        ...
        lib.nameValuePair "rclone-sync-${name}" {
          secrets.config-file.encrypted.file = config.environment.rclone-target.configFile;
        }
        ...
        lib.nameValuePair "rclone-sync-${name}" {
          ...
          serviceConfig = {
            ...
            ExecStart = "${pkgs.rclone}/bin/rclone --config ${
              config.secrix.services."rclone-sync-${name}".secrets.config-file.decrypted.path
            } bisync --resilient --recover --max-lock 2m --conflict-resolve newer --check-access ${target.filePath} ${target.remoteName}";
      ...
      // lib.mapAttrs' (
        ...
        lib.nameValuePair "rclone-sync-${name}-resync" {
          ...
          serviceConfig = {
            Type = "oneshot";
            ExecStart = "${pkgs.rclone}/bin/rclone --config ${
              config.secrix.services."rclone-sync-${name}-resync".secrets.config-file.decrypted.path
            } bisync --resync --resilient --recover --max-lock 2m --conflict-resolve newer --check-access ${target.filePath} ${target.remoteName}";
            User = "John88"; # Adjust user as needed
          };
        }
      ) config.environment.rclone-target.targets;
