...
let
  ...
  planes = let
    ...
    withPeers = foldl' (acc: host:
      ...
      ) acc (host.coordinate or [])
    ) base (attrValues hosts);
  in
  ...
  vParentResolution =
    let
      ...
      hubIndex = foldl' (acc: e:
        acc // { ${planeKey e.plane_name e.subnet} = e.hub; }
      ) {} allHubOfEntries;
    in
      filter (x: x != null) (flatten (map (host:
        map (coord:
  ...
  vPeerIdUniqueness =
    let
      groups = foldl' (acc: host:
        foldl' (innerAcc: coord:
          let
  ...
  vIcmpOverrideInterfaces =
    let
      allCoordInterfaces = unique (flatten (map (host:
