...
let
  ...
  warnings = lib.flatten (
    lib.mapAttrsToList (hostname: machine:
  ...
  settings = lib.mapAttrs (hostname: machine:
    let
    ...
    if pubKey == null then null else {
      ...
      peers = if isHub then
        ...
        lib.filter (x: x != null) (
          lib.mapAttrsToList (peerName: peerMachine:
        ...
        let hubMachine = topology.${hubName};
