...
}:
{
  ...
  config = lib.mkIf config.environment.vpn.enable {
    ...
    secrix.services.wireguard-wireg0.secrets."${config.environment.vpn.hostname}".encrypted.file =
      lib.mkIf (config.environment.vpn.privateKeyFile == null)
        "${self}/secrets/private_keys/wireguard/wg_${config.environment.vpn.hostname}";
      ...
      interfaces = {
        wireg0 = {
          ...
          privateKeyFile = lib.mkMerge [
            ...
            (lib.mkIf (config.environment.vpn.privateKeyFile != null) config.environment.vpn.privateKeyFile)
          ];
          peers = [
            {
