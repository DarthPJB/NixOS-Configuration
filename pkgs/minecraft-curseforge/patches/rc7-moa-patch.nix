# Aether moa_type registry fix for all-the-mons 1.0.0-rc.7
#
...
#
# *** MUST BE REVALIDATED when updating past rc.7 ***
#
# Carry-forward from rc.6 — same Aether version expected in rc.7.
# If Aether is updated in a future release and the bug is fixed,
# this patch can be removed entirely.
...
let
  ...
  _versionCheck =
    assert lib.assertMsg (version == "1.0.0-rc.7")
      "rc7-moa-patch: expected modpack version '1.0.0-rc.7', got '${version}'. This patch must be revalidated for the new version.";
    true;
in
